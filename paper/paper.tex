...
204 theorems and lemmas across 17 files.
...
and path separation (the score--path distinction underlying the
$\bigO(m\varepsilon)$ extraction).
Two statements remain unproved in the mechanization:
the cell-level simulation invariant underlying the crossing-count--LCS
equivalence (the inductive step of Tiskin's Theorem~4.10~\cite{tiskin2022book}),
and two bridge lemmas connecting the correction formula to the concrete Gotoh~DP
computation (the column-recursive/fold equivalence and the upper bound
when $\varepsilon \le g_o$).
...
reduces to the two bridge lemmas via the diagonal feasibility argument.
All unproved statements are verified exhaustively by testing
(174{,}000 and 17 cases respectively with zero failures)
but their full formal proofs require novel techniques for
relating fold-based list computations to recursive cell definitions.
