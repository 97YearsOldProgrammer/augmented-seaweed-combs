...
\usepackage{setspace}
\setstretch{1.05} % Slight increase in line spacing for readability

% --- Modern Typography ---
\usepackage[T1]{fontenc}
\usepackage{amsmath,amsthm,amssymb} % Load AMS before fonts to avoid \Bbbk clash
\usepackage{newpxtext, newpxmath} % Elegant Palatino-style font
\usepackage{microtype} % Optical margin alignment and font expansion
\usepackage{inconsolata} % Cleaner monospace font for URLs and code
\usepackage{url}
\Urlmuskip=0mu plus 3mu % Allow URL line breaks
...
\titleformat{\subsection}{\large\bfseries\color{OxfordBlue}}{\thesubsection}{1em}{}

% --- Standard Premium Theorem Environments ---
...
% --- Title Information ---
\title{\vspace{-2em}\color{OxfordBlue}\textbf{Augmented Semi-Local String Comparison:\\Algorithms and Applications}}
\author{\textbf{Gong Chen} \\
\normalsize UC Davis Genome Center, University of California, Davis\\
\normalsize \href{mailto:dsbchen@ucdavis.edu}{\texttt{dsbchen@ucdavis.edu}}}
\date{\vspace{-2em}}
...
\begin{abstract}
Tiskin's semi-local framework offers composable $\bigO(n \log n)$ string comparison, but affine gap penalties---essential for biological sequence alignment---have required a costly sentinel blow-up ($\nu^2$ expansion in string length).
We resolve this by introducing the \emph{augmented seaweed comb}: each entry of the distance column~$\dcol$ carries a gap depth counter that resets on match and increments on mismatch.
The augmented output determines the complete Gotoh DP state---all three matrices $H$, $E$, $F$.
The algebraic structure is the wreath product $\mathfrak{S}_{m+n} \wr \mathrm{Aff}(\mathbb{Z}_{\geq 0})$, with $\bigO(n \log n)$ composition.
We establish an exact bijection with the sentinel blow-up, achieving $2\times$ storage overhead versus $\nu\times$, and prove that the semi-local affine gap score matrix is not Monge, confirming the wreath product extension is necessary.
\end{abstract}

\section{Introduction}\label{sec:intro}

The \emph{semi-local longest common subsequence} (LCS) problem asks: given strings $A$ of length~$m$ and $B$ of length~$n$ over a finite alphabet~$\Sigma$, compute $\LCS(A', B')$ for all contiguous substring pairs.
Tiskin~\cite{tiskin2006semi,tiskin2008semi} showed that the complete answer---comprising $\Theta(mn)$ distinct scores---is encoded by a single permutation on $m{+}n$ elements, composable via \emph{distance multiplication} in $\bigO(n \log n)$ time.

Krusche~\cite{krusche2010spaa} and Krusche and Tiskin~\cite{krusche2009plots} reformulated this into the \emph{distance column}~$\dcol$, a vector of~$n$ integers that can be composed incrementally in $\bigO(n)$ per query character.
This composability makes the distance column attractive for seed-and-extend alignment pipelines, where thousands of candidate windows must be scored against a common reference~\cite{tiskin2008semi}.

All practical sequence aligners penalize gaps---insertions and deletions that interrupt the alignment.
The natural question is whether the distance column suffices for gap-penalized scoring.
Tiskin's sentinel blow-up~\cite{tiskin2008semi,tiskin2019bounded} provides one answer: replace each character~$c$ with $\$^g c$ where~$\$$ is a fresh sentinel, compute the standard semi-local LCS on the expanded strings, and read off gap-penalized scores from the expanded distance column of length~$n\nu$ with $\nu = g{+}1$.
This works, but the $\nu^2$ expansion in string length is costly.
The distance pair $(\dcol, \drow)$ is provably insufficient: pairs of inputs with identical $(\dcol, \drow)$ produce distinct gap-penalized scores, and $\drow$ is not preserved under composition since row distances are overwritten during the comb sweep.
The blow-up has therefore remained the only known route to semi-local gap scoring.

We resolve the affine gap scoring problem by introducing the \emph{augmented seaweed comb}.
Each wire in the Krusche comb carries a \emph{gap depth counter}---a non-negative integer that resets to zero on match and increments by one on mismatch.
The augmented output $(\dcol, \delta_{\mathrm{col}}, \delta_{\mathrm{row}})$ determines the complete Gotoh DP state---all three matrices $H$, $E$, $F$---at every window position.
The depth counter is a pure observer that does not affect label comparisons, so the standard permutation is preserved.
The algebraic structure is the wreath product $\mathfrak{S}_{m+n} \wr \mathrm{Aff}(\mathbb{Z}_{\geq 0})$, with $\bigO(n \log n)$ composition.
We establish an exact bijection with the sentinel blow-up at
constant overhead independent of the gap penalty, and prove that
Monge-based composition cannot extend to affine gap scoring.
Key results are formally verified in Lean~4; proof scripts are
available at
\url{https://github.com/97YearsOldProgrammer/augmented-seaweed-combs}.

\begin{sloppypar}
Current sequence
aligners---including minimap2~\cite{li2018minimap2},
STAR~\cite{dobin2013star}, and
DIAMOND~\cite{buchfink2015diamond}---score each candidate window
via full Gotoh~DP in $\bigO(m^2)$ time per window.  The augmented
comb replaces this with a single $\bigO(mn)$ build whose output
scores all windows simultaneously, with per-window extraction in
$\bigO(m\varepsilon)$ where $\varepsilon$ is the excess of LCS
over diagonal matches and is typically small for biological
sequences.
\end{sloppypar}

\begin{sloppypar}
The semi-local framework and its extensions have attracted
sustained interest.
Tiskin's monograph~\cite{tiskin2022book} consolidates the theory
of seaweed braids, distance multiplication, and the sentinel
blow-up for bounded-length Smith--Waterman
alignment~\cite{tiskin2019bounded}.  State-of-the-art algorithms
for weighted sequence alignment uniformly rely on the blow-up or
its structural equivalent:
Cassis, Kociumaka, and Wellnitz~\cite{cassis2023weighted} achieve
optimal bounded weighted edit distance via blow-up and Tiskin
composition; Gorbachev and
Kociumaka~\cite{gorbachev2025bounded} extend this to integer
weights using core-sparse Monge
multiplication~\cite{gawrychowski2025coresparse};
Charalampopoulos, Kociumaka, and
Mozes~\cite{charalampopoulos2020dynamic} use the full Tiskin
framework for dynamic string alignment.  On the structural side,
Russo~\cite{russo2012monge} established Monge properties of
gap-penalized score matrices, and Carmel, Tsur, and
Ziv-Ukelson~\cite{carmel2016almost} studied almost-Monge
structure in alignment scoring.  The augmented comb provides an
alternative that avoids the $\nu^2$ expansion entirely, while
preserving the composability that makes the Tiskin framework
algorithmically useful.
\end{sloppypar}

Section~\ref{sec:prelim} recalls the Tiskin--Krusche framework and the Gotoh affine gap model.
Sections~\ref{sec:augmented}--\ref{sec:extraction} introduce the augmented comb, its wreath product algebraic structure, and score extraction.
Algorithms~\ref{alg:augmented-build}--\ref{alg:banded-gotoh} provide complete pseudocode for the augmented comb build, wreath product composition, adaptive score extraction, and $\varepsilon$-banded path extraction.
Section~\ref{sec:non-monge} establishes the non-Monge negative results.  Section~\ref{sec:blowup} proves the bijection with the sentinel blow-up.  Section~\ref{sec:conclusion} concludes.

\section{Preliminaries}\label{sec:prelim}

We follow the notation of Tiskin~\cite{tiskin2008semi}.  Let
$A = a_0 a_1 \dots a_{m-1}$ and $B = b_0 b_1 \dots b_{n-1}$ be
strings over a finite alphabet~$\Sigma$ of size~$\alpha$.
We
write $B[s{:}t]$ for the substring $b_s b_{s+1} \dots b_{t-1}$.
The \emph{tropical semiring} is
$\Trop = (\Real \cup \{-\infty\}, \max, +)$.

Informally, $m{+}n$ wires pass through the $m \times n$ comparison
grid; at each mismatch, adjacent wires swap if out of order.
The
final wire permutation encodes all semi-local LCS scores
simultaneously.

\begin{definition}[Seaweed braid and permutation]
\label{def:seaweed}
The \emph{seaweed braid} for $(A, B)$ operates on
$m{+}n$ wires indexed $0, 1, \dots, m{+}n{-}1$.
At cell
...
has $\ell_t < \ell_{t+1}$, the wires swap;
otherwise they do
...
partitions~\cite{tiskin2008semi}.
\end{definition}

\begin{figure}[!htbp]
\centering
\input{figures/fig_krusche_comb}
\caption{The Krusche comb for $A = \texttt{AB}$, $B = \texttt{ABBA}$.}
\label{fig:krusche}
\end{figure}

The braid permutation encodes LCS scores implicitly;
to make them
explicit we use the \emph{Krusche comb}, an incremental
reformulation due to Krusche~\cite{krusche2010spaa} that replaces
wire tracking with a pair of distance vectors.
Intuitively,
$\dcol[c]$ records how far into the query the crossings at reference
position~$c$ have reached.
...
$\drow[0{:}m]$ initialized to $\drow[i] = i{+}1$.
Processing
...
one.
The affine model of Gotoh~\cite{gotoh1982improved}
...
Gap-penalized scoring requires information beyond the plain distance
vectors.
The sentinel blow-up of Tiskin~\cite{tiskin2008semi}
provides one solution at the cost of a~$\nu^2$ expansion.
The key
observation is that the blow-up's $\nu$ extra wires per character
encode only a one-dimensional quantity---the consecutive mismatch
count at each position.
A single integer counter per wire captures
...
(standard sort).
Depths follow their swapped labels and
increment:
...
In the mismatch-swap case, depth travels with the wire identity:
a wire that has been mismatching for $k$~steps carries that history
even after being rerouted by a swap.
\end{definition}
...
\For{$r = 0$ \KwTo $m{-}1$}{
  \For{$c = 0$ \KwTo $n{-}1$}{
    ...
    \Else(\tcp*[f]{Mismatch, no swap}){
      $\delta_{\mathrm{col}}[c] \leftarrow \hat{\delta}_c + 1$;
      $\delta_{\mathrm{row}}[r] \leftarrow \hat{\delta}_r + 1$\;
    }
...
\noindent
\textbf{Implementation note.}  In the mismatch-swap case (line~11--12), the depth assignments use the \emph{pre-swap} depth values $\hat{\delta}_r$ and $\hat{\delta}_c$ saved at line~5.
A na\"ive implementation that reads $\delta_{\mathrm{row}}[r]$ after the label swap has already written to $\delta_{\mathrm{col}}[c]$ would use corrupted values.  The temporary variables $\hat{d}$ and $\hat{\delta}$ resolve this.

The label rules are identical to the standard Krusche comb.
Depth is a pure \emph{observer}: it does not affect any label
comparison or swap decision, so
...
occurred since the last match:
a gap has just opened, incurring
...
\begin{figure}[!htbp]
\centering
\input{figures/fig_augmented_comb}
...
is incremented once per column.
Thus the triple
$(\dcol, \delta_{\mathrm{col}},
\delta_{\mathrm{row}})$ suffices.
\end{proof}

\begin{remark}[Semi-local scope]
\label{rem:semi-local}
Theorem~\ref{thm:score-determination} determines the \emph{semi-local}
affine gap score (Definition~\ref{def:affine-gap}), where the reference
endpoint is free.
The standard global Gotoh score---where both endpoints
are fixed and each $k$-gap costs $g_o + (k{-}1)g_e$---is a different
quantity that the comb does not determine in general, since distinct
LCS-optimal paths may incur different affine gap costs.
\end{remark}
...
$\{\dcol, \delta_{\mathrm{col}}, \drow,
\delta_{\mathrm{row}}\}$ for affine gap scores.
The
...
$(A_1, B_1) = (0110,\allowbreak 10101010)$ (score~$2$) and
$(A_2, B_2) = (0010,\allowbreak 10101010)$ (score~$3$) yield
identical $(\dcol, \delta_{\mathrm{col}})$ but differ in
$\delta_{\mathrm{row}}$.

Dropping $\delta_{\mathrm{col}}$:
$(A_1, B_1) = (0011,\allowbreak 10101010)$ (score~$2$) and
$(A_2, B_2) = (0001,\allowbreak 10101010)$ (score~$3$) yield
...
Each of the three components is individually necessary;
the
...
The composability of the distance column is what makes Tiskin's
framework useful in pipelines.
We now show that the augmented comb
inherits this composability: the depth transformations form a monoid,
and the combined label-plus-depth dynamics is captured by a wreath
product with $\bigO(n \log n)$ composition.
...
The identity element is $f_{1,0}$.
\end{definition}
...
\delta + 1$.
A wire traverses a sequence of cells,
...
mismatches each add~$1$);
if the wire never matched, the net
...
algorithm~\cite{tiskin2008semi} in $\bigO(n \log n)$.
The
...
actual numeric score requires additional computation.
We now
...
introduces at least one gap.
Each gap opening costs at
least~$g_o$, so the total gap penalty is at least~$g_o$.
The
gap-free diagonal alignment achieves
$d = \sum_{i} \mathbf{1}[a_i = b_{s+i}]$ matches with zero gap
cost.
Any gapped alignment can gain at most $m - d$ additional
matches but pays at least~$g_o$.
Since
...
by one position, over an alphabet of size~$m$.
The diagonal
...
crossings, each opening a gap at cost at least~$g_o$.
Since the
...
the LCS would be higher).
Each off-diagonal crossing therefore
...
$g_o = 2$, $g_e = 1$.
Both have $\LCS = 3$ and diagonal
match count~$2$, giving $\varepsilon = 1$.
However, the
...
so~$P$ is strictly suboptimal.
The optimal path therefore has
...
pass over $m$ positions.
Step~(2) is $\bigO(1)$: a comparison
...
Three negative results bound this problem:
\begin{enumerate}
  \item \textbf{No per-position decomposition.}
    $f$ cannot be written as $\sum_j h(j, \dcol[j],
    \delta_{\mathrm{col}}[j])$: the expanded crossing count at
    position~$j$ depends on neighboring positions.
  \item \textbf{No bounded-state scan.}
    The map $(\dcol, \delta_{\mathrm{col}}) \to \dcol_\nu$
    requires carry state of dimension~$2m$ (the expanded row
    vector), not $\bigO(1)$.
  \item \textbf{Intermediate state asymmetry.}
    The final column state determines $\dcol_\nu$ via the
    bijection, but the intermediate row state
    $(\drow, \delta_{\mathrm{row}})$ does \emph{not} determine
    $\drow_\nu$ when $\nu > 2$.
\end{enumerate}
These suggest that worst-case $\bigO(m^2)$ extraction may be
unavoidable, consistent with the non-Monge structure of
Section~\ref{sec:non-monge}.
However, the $\varepsilon$-banded
DP provides an adaptive bypass in $\bigO(m\varepsilon)$
(Corollary~\ref{cor:adaptive-extraction}).
\end{remark}

The adaptive extraction (Corollary~\ref{cor:adaptive-extraction}) is exact for all inputs, but its practical utility depends on how often the excess $\varepsilon$ is small.
The following result provides a structural explanation.
...
parameters (Proposition~\ref{prop:tropical-matrix}).
If
...
$F_j \leq c_j - 1$ is maintained at each step.
At column~$j$,
the propagated state is
$p_j = \max(H_{j-1} - g_o,\, F_{j-1} - g_e)$.
Under the
...
alignment.
Empirically, $\varepsilon = 0$ implies perfect
...
\begin{proposition}[Excess bounds]
\label{prop:excess-bounds}
\begin{enumerate}
  \item $\varepsilon \leq |\{(i, j) : a_i = b_{s+j},\; i \neq j\}|$
    (off-diagonal matchable positions).
  \item For reference decomposition $B = B_1 B_2$, the excess
    satisfies $\varepsilon(B) \geq \varepsilon(B_1) + \varepsilon(B_2)$
    (super-additivity).
\end{enumerate}
\end{proposition}

\begin{proof}
Part~(1): any LCS alignment achieving $\varepsilon$ off-diagonal
matches must place each on a distinct off-diagonal position $(i,j)$
with $a_i = b_{s+j}$ and $i \neq j$, so $\varepsilon$ is bounded
by the number of such positions.
Part~(2): restricting an LCS alignment of~$A$ against~$B$ to each
reference half yields valid common subsequences.
Diagonal matches
are additive over the split, while the global LCS is super-additive,
so the excess $\varepsilon = \LCS - \mathrm{diag}$ is super-additive.
\end{proof}

\section{Non-Monge Structure}\label{sec:non-monge}

The augmented comb provides an affine gap framework via the
wreath product rather than via Monge structure.
We now show
...
Monge nor unit-Monge, even for $m{=}2$.
This contrasts with
LCS, where $S$ is always unit-Monge.
\end{theorem}

\begin{proof}
We exhibit a counterexample.
Let $A = 01$, $B = 0001$,
$g_o = 3$, $g_e = 1$.
The semi-local score matrix
...
structure.
The structural reason is that gap penalties cause the
...
\begin{proof}
The Gotoh recurrence at column~$j$ updates the state vector
$(H_j, F_j, 1)$ via
$H_j = \max(F_j, c)$, $F_j = \max(H_{j-1} - g_o, F_{j-1} - g_e)$,
which is tropical-linear with the stated matrix.
The semigroup is infinite because the parameter~$c$ varies
freely, and the products do not collapse to a finite set.
\end{proof}
...
for gap-penalized scoring.
We now show they are informationally
equivalent.
...
uses $\nu(m{+}n)$.
For $g_o = 3$: the augmented comb achieves
...
and $B' = \$^g b_0 \$^g b_1 \cdots$.
The blown-up comb
operates on $\nu(m{+}n)$ wires, grouped into $\nu$-tuples per
original position.
Within each group, the $g$ sentinel
...
gap depth $\delta_{\mathrm{col}}[j]$.
To see this: at a match
($a_r = b_c$), all $\nu$ sub-wires in the group swap together,
resetting the sentinel crossing count to zero.
At a mismatch,
the $g$ sentinel sub-wires see $\$ \ne b_c$ (or $a_r \ne \$$)
and one additional pair crosses, incrementing the count by
exactly one.
After $\delta$ consecutive mismatches, the sentinel
crossing count is $\min(\delta, g) = \delta$ (since
$\delta \leq m < \nu$ for meaningful inputs).
This encoding is injective, so the
...
\begin{figure}[!htbp]
\centering
\resizebox{\linewidth}{!}{\input{figures/fig_blowup}}
...
determines $\dcol_\nu$ with zero ambiguity.
The row vectors
$\drow$ and $\delta_{\mathrm{row}}$ are not required for
reconstructing the expanded distance column.
Furthermore, the
...
information.
The prefix-computability is immediate: the first
...
encoding character equality.
This interleaved pattern lets
...
The sentinel sub-wires implicitly
...
on the original (unexpanded) strings.
However, $\dcol_\nu$
...
$\nu = 1$ and $\dcol_\nu = \dcol$.
LCS scores depend only on
$\dcol$, and the row distances $\drow$ are determined by $\dcol$
up to the conservation constraint.
Thus $\dcol_\nu$ and
...
identities.
The bijection of Theorem~\ref{thm:bijection}
...
extraction under any affine gap model.
By contrast, the sentinel
blow-up must be recomputed for each value of~$g$ at
$\bigO(mn\nu^2)$ cost per setting.
...
\begin{sloppypar}
The augmented seaweed comb extends Tiskin's semi-local string
comparison framework to affine gap alignment.
The central positive result is that the augmented output
$(\dcol, \delta_{\mathrm{col}}, \delta_{\mathrm{row}})$---three
vectors totaling $2(m{+}n)$ integers---determines the complete
Gotoh DP state exactly, and composition via the wreath product
$\mathfrak{S}_{m+n} \wr \mathrm{Aff}(\mathbb{Z}_{\geq 0})$
retains $\bigO(n \log n)$ complexity.
The exact bijection with the sentinel blow-up
(Theorem~\ref{thm:bijection}) establishes informational
equivalence at constant storage overhead, independent of the gap
penalty.  On the negative side, the non-Monge structure of the
semi-local affine gap score matrix
(Theorem~\ref{thm:non-monge}) confirms that the wreath product
is necessary: the standard Tiskin framework based on unit-Monge
composition does not extend to gap-penalized scoring.

The principal remaining question is the score extraction formula.
The map from augmented output to affine gap score exists
(Theorem~\ref{thm:score-determination}), and the adaptive
$\varepsilon$-banded DP achieves $\bigO(m\varepsilon)$ extraction
(Corollary~\ref{cor:adaptive-extraction}), but a closed-form
$\bigO(m)$ formula remains open
(Remark~\ref{rem:open-problem}).  The three negative results on
extraction suggest that $\bigO(m^2)$ may be the best worst-case
bound, though the adaptive scheme is $\bigO(m)$ whenever
$\varepsilon$ is bounded.
An $\bigO(m)$ formula would make the augmented comb strictly superior to the blow-up for all operations.

We note that the augmented comb determines the \emph{semi-local} affine gap score (Remark~\ref{rem:semi-local});
the standard global Gotoh score, where both endpoints are fixed, is a different quantity not determined by the comb in general.
For practical alignment pipelines, the augmented comb can replace the per-window $\bigO(m^2)$ Gotoh DP currently used by seed-and-extend aligners, scoring all windows from a single $\bigO(mn)$ comb build with $\bigO(m\varepsilon)$ adaptive extraction per window.
Parallel composition on GPU architectures could exploit the wreath product structure for further speedups, since the pointwise affine application is embarrassingly parallel.
Bridging the semi-local score to the standard global Gotoh DP---where both endpoints are fixed and each $k$-gap costs $g_o + (k{-}1)g_e$---remains open; the comb does not encode which gap arrangement minimizes the full affine penalty.

\paragraph{Formal verification.}
All theorems and propositions in this paper are formalized in
Lean~4 with Mathlib across 24 sorry-free files covering the
augmented comb, wreath product algebra, score extraction,
non-Monge structure, and the sentinel blow-up bijection.
A companion file explores the semi-local to standard global
bridge; three open proof obligations there confirm that this
connection lies beyond the augmented comb's scope.
\end{sloppypar}
...
\newblock \emph{Journal of Discrete Algorithms}, 6(4):570--581, 2008.
...
A.~Dobin, C.~A. Davis, F.~Schlesinger, J.~Drenkow, C.~Zaleski,
  S.~Jha, P.~Batut, M.~Chaisson, and T.~R. Gingeras.
\newblock {STAR}: ultrafast universal {RNA}-seq aligner.
\newblock \emph{Bioinformatics}, 29(1):15--21, 2013.

\bibitem{buchfink2015diamond}
B.~Buchfink, C.~Xie, and D.~H. Huson.
\newblock Fast and sensitive protein alignment using {DIAMOND}.
\newblock \emph{Nature Methods}, 12:59--60, 2015.
