...
256 theorems and lemmas across 20 files.
...
Four statements remain unproved in the mechanization, across two files.
Three are sub-goals of \texttt{colSimInv\_step}, the cell-level inductive
step underlying the crossing-count--LCS equivalence (Tiskin's
Theorem~4.10~\cite{tiskin2022book}).  Each corresponds to one branch of the
comb's column step---match, mismatch-with-swap, and mismatch-with-stay---and
must establish that the comb's row displacement \texttt{d\_row\_k} at the
newly processed position satisfies
$\mathtt{d\_row\_k} > j \iff \mathit{dp\_new}[j{+}1] > \mathit{dp\_new}[j]$,
connecting comb wire position to DP cell comparison.
The inductive infrastructure is complete: DP prefix locality
(\texttt{lcsDpRow\_getD\_prefix}, proved via fold-level agreement) and
per-position \texttt{CombEncodes} (proved via width subtraction) supply the
supporting lemmas; only the final row-displacement--to--DP bridge at the
new position remains open.  All three sub-goals are verified exhaustively
(1.17~million cases, zero failures).
The fourth sorry is a bridge lemma for score determination:
$\mathrm{gotohGlobalScore} \le \mathrm{diagCount}$ when $\varepsilon \le g_o$
(\texttt{gotoh\_le\_diag\_when\_eps\_small} in \texttt{ScoreDetermination.lean}),
whose mathematical argument (any gapped path pays $\ge g_o$ and gains
$\le \varepsilon \le g_o$) is sound but whose formalization requires alignment
path decomposition over the fold-based Gotoh~DP.
The score determination theorem is structurally proved: the
$\varepsilon > g_o$ case is definitional, and the $\varepsilon \le g_o$ case
reduces to two bridge lemmas---the lower bound (diagonal path feasibility,
fully proved) and the upper bound (the remaining sorry)---via \texttt{linarith}.
All four unproved statements are verified exhaustively by testing
(1.17~million and 174{,}000+ cases respectively with zero failures)
but their full formal proofs require novel techniques for
relating fold-based list computations to comb wire positions and alignment
path properties.
