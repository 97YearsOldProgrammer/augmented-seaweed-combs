...
232 theorems and lemmas across 20 files.
...
Four statements remain unproved in the mechanization:
three sub-lemmas of the cell-level simulation invariant underlying the
crossing-count--LCS equivalence (the inductive step of Tiskin's
Theorem~4.10~\cite{tiskin2022book}), requiring a row-displacement invariant
to connect comb wire positions to DP cell values;
and one bridge lemma for score determination: the upper bound
$\mathrm{gotohGlobalScore} \le \mathrm{diagCount}$ when $\varepsilon \le g_o$,
whose mathematical argument (any gapped path pays $\ge g_o$ and gains $\le
\varepsilon \le g_o$) is sound but whose formalization requires alignment path
decomposition over the fold-based Gotoh~DP.
...
(174{,}000+ and 30+ cases respectively with zero failures)
but their full formal proofs require novel techniques for
relating fold-based list computations to alignment path properties.
